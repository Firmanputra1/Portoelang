    ...
    <form action="{{ route('admin.portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <div style="margin-top: 8px;">
                <img src="{{ (config('filesystems.default') === 'local' || config('filesystems.default') === 'public') ? asset('storage/' . $portfolio->image) : Storage::disk(config('filesystems.default'))->url($portfolio->image) }}" alt="Preview" style="max-width: 200px; max-height: 150px; border-radius: 8px; border: 1px solid var(--border);">
            </div>
        </div>
        @endif
